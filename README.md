...
Highest priority is getting the mod to some runnable state on the "modern" version, 1.14+(some letter of the alphabet) but the procedure will remain the same:
1. Take measures to preserve the existing patch_d2.mpq (unless you never want to play vanilla again without reinstalling)
2. Overwrite the vanilla patch_d2.mpq with the modded one.
3. Run the game as normal.

alternatively, copy the data folder into the same folder as "Diablo II.exe" and start the game using the -txt command-line parameter. 

i.e. "C:\Program Files\Diablo II\Diablo II.exe" -txt

This avoids messing around with the patchfile, and vanilla D2 can be played by lauching without command-line options.

## Added/Changed Items (should prolly use the wiki for this)
### Runes & Runewords
